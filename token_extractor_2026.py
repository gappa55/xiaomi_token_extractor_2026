...
class XiaomiCloudConnector:
    ...
    def login(self):
        # Pre-seed cookies like the Android app does
        ...
        # If step 2 returned a direct location (no 2FA) we'll follow it here.
        if self._location and not self._serviceToken:
            if not self.follow_location_and_extract_tokens(self._location):
                print_if_interactive("Unable to get service token.")
                return False

        # Double-check we have everything we need
        if not self._ssecurity:
            _LOGGER.error("Missing ssecurity after login; cannot call encrypted API.")
            return False
        if not self._serviceToken:
            _LOGGER.error("Missing serviceToken after login; cannot call API.")
            return False
    ...
    def login_step_1(self):
        ...
        _LOGGER.debug("step1 URL: %s status=%s", response.url, response.status_code)
        _LOGGER.debug("step1 body: %s", response.text[:1000])
        valid = response.status_code == 200 and "_sign" in self.to_json(response.text)
        if valid:
            self._sign = self.to_json(response.text)["_sign"]
            _LOGGER.debug("Obtained _sign: %s...", self._sign[:3] + "...")
        return valid
    ...
    def login_step_2(self) -> bool:
        ...
        _LOGGER.debug("login_step_2 fields=%s", {**fields, "hash": "***"})

        response = self._session.post(url, headers=headers, params=fields, allow_redirects=False)
        _LOGGER.debug("login_step_2 status=%s body=%s", response.status_code, response.text[:1000])

        if response is None or response.status_code != 200:
            _LOGGER.error("login_step_2: HTTP status: %s; Response: %s", 
                          getattr(response, "status_code", None), getattr(response, "text", "")[:500])
            return False
        ...
        if "captchaUrl" in json_resp and json_resp["captchaUrl"]:
            ...
            _LOGGER.debug("Retrying login with captcha (fields sanitized).")
            response = self._session.post(url, headers=headers, params=fields, allow_redirects=False)
            _LOGGER.debug("login_step_2 (captcha retry) status=%s body=%s", response.status_code, response.text[:1000])
            if response is None or response.status_code != 200:
        ...
        # Success path without 2FA
        if "ssecurity" in json_resp and json_resp.get("ssecurity"):
            ...
            _LOGGER.debug("Direct login success. ssecurity present. location=%s", self._location)
            return True

        # 2FA required
        if "notificationUrl" in json_resp and json_resp["notificationUrl"]:
            _LOGGER.debug("Two-factor authentication required. Starting 2FA email path.")
            return self.do_2fa_email_flow(json_resp["notificationUrl"])
    ...
    def do_2fa_email_flow(self, notification_url: str) -> bool:
        ...
        self._log_cookies("before authStart")
        ...
        self._log_cookies("before identity/list")
        ...
        self._log_cookies("after identity/list")
        ...
        self._log_cookies("before sendEmailTicket")
        ...
        self._log_cookies("after sendEmailTicket")
        try:
            jr = r.json()
        except Exception:
            jr = {}
        ...
        code = input("Enter the Xiaomi email verification code (check inbox/spam): ").strip()
        verify_params = {
            "_flag": "8",
            "_json": "true",
            "sid": "xiaomiio",
        ...
        self._log_cookies("before verifyEmail")
        ...
        self._log_cookies("after verifyEmail")
        ...
        self._log_cookies("before finish_2fa chain")
    ...
    def follow_location_and_extract_tokens(self, location_url: str) -> bool:
        headers = {
            "User-Agent": self._agent,
            "Content-Type": "application/x-www-form-urlencoded"
        }
        _LOGGER.debug("Following location to finalize login: %s", location_url)
        r = self._session.get(location_url, headers=headers, allow_redirects=True)
        _LOGGER.debug("login_step_3 follow location status=%s", r.status_code)
        if r.status_code != 200:
            return False
        self._serviceToken = self._session.cookies.get("serviceToken")
        if not self._serviceToken:
            return False
        self.install_service_token_cookies(self._serviceToken)
        return True
    ...
    def execute_api_call_encrypted(self, url, params):
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                _LOGGER.debug("Failed to parse JSON from decrypted payload: %s", decoded[:200])
                return None
        _LOGGER.debug("API call failed: status=%s body=%s", response.status_code, response.text[:200])
        return None
    ...
    def install_service_token_cookies(self, token: str):
        ...
        _LOGGER.debug("Installed serviceToken for API domains; snapshot: %s", self._dump_cookies())

    def _dump_cookies(self):
        return [{"name": c.name, "domain": c.domain, "path": c.path, "secure": c.secure} for c in self._session.cookies]

    def _log_cookies(self, label):
        _LOGGER.debug("Cookie snapshot [%s]: %s", label, self._dump_cookies())
    ...
    def handle_captcha(self, captcha_url: str) -> str:
        # Full URL in case it is relative
        ...
        try:
            ...
            _LOGGER.debug("Captcha server failed: %s", e1)
            ...
            try:
                ...
                _LOGGER.debug("Opening image failed: %s", e2)
...
___ ____ _  _ ____ _  _ ____    ____ _  _ ___ ____ ____ ____ ___ ____ ____ 
 ...
 |  |__| | \_ |___ | \| ___]    |___ _/\_  |  |  \ |  | |___  |  |__| |  \ 
                            by Piotr Machowski , 2FA enhancments by Zlatko 
